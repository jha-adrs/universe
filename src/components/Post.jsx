            ...
            <div className="px-4 py-3 flex flex-col sm:flex-row">
                ...
                <div className="flex-1 mt-3 sm:mt-0">
                    <div className="text-xs text-gray-500 dark:bg-zinc-800 dark:text-white">
                        {communityName && (
                            <a href={`/r/${communityName}`} className='underline text-zinc-900 dark:bg-zinc-800 dark:text-white text-sm underline-offset-2'>
                                r/{communityName}
                            </a>
                        )}
                        <span className="px-1"></span>
                        <span className="text-gray-900 dark:bg-zinc-800 dark:text-white">
                            <a href={`/u/${post.author.username}`} className="text-blue-800 dark:text-white">
                    ...
                    <a href={`/r/${communityName}/post/${post.id}`}>
                        <h1 className="text-lg font-semibold py-2 leading-6 text-gray-900 dark:bg-zinc-800 dark:text-white">
                            {post.title}
                        </h1>
                    </a>

                    <div className='relative text-sm max-h-40 overflow-clip' ref={pRef}>
                        <EditorOutput className='dark:bg-zinc-800 dark:text-white' content={post.content}/>
                        {pRef.current?.clientHeight === 200 && (
                            <div className="absolute bottom-0 left-0 h-35 w-full bg-transparent"/>
                        )}
                    </div>
                </div>
            </div>
            <div className="bg-gray-50 dark:bg-zinc-800 dark:text-white z-20 p-3 sm:px-4">
