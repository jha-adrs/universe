...
const UserAuthForm = ({ className, ...props }) => {
    ...
    return (
        <div>
            ...
            <Button onClick={loginWithMagicLink} className={cn('flex justify-center m-2', className)} disabled={isLoading}>
            ...
            <div className="flex justify-center items-stretch m-2">
                <Button onClick={loginWithGoogle} variant='outline' className={cn('flex m-2', className)} disabled={isLoading}>
                    {isLoading ? <Icons.spinner className='w-5 h-5 mr-2 animate-spin' /> : <Icons.google className='w-5 h-5 mr-2' />} Google
                </Button>
                <Button variant='outline' className={cn('flex m-2', className)} disabled={isLoading}>
